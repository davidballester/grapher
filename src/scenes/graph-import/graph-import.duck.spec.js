...
describe('graph-import', () => {
  ...
  describe('sagas', () => {
    ...
    describe(doImportGraph.name, () => {
      ...
      it('puts a create graph action if the deserialization offers no errors', () => {
        const graph = { foo: 'bar' };
        const gen = cloneableGenerator(doImportGraph)(action);
        gen.next();
        gen.next({ graph });
        const createGraphAction = createGraph(graph);
        // The ID is autogenerated. Comparing them will fail
        delete createGraphAction.payload.id;
        expect(gen.next().value).toMatchObject(put(createGraphAction));
      });
    });
  });
});
